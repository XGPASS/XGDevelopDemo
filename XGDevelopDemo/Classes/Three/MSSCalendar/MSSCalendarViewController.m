...
- (instancetype)init {
    ...
    if(self){
        ...
        _showAlertView = NO;
        _showType = MSSCalendarShowTypeIsPresent;
    }
    return self;
}
...
- (void)viewDidLoad {
    ...
    if (self.navigationController) self.isPush = YES; // 判断是否有nav
    [self initDataSource];
    [self createUI];
    [self addBackItem];
}
...
- (void)addWeakView {
    CGFloat posY = self.isPush ? 64.0 : 0.0;
    CGFloat statusHeight = self.isPush ? 0.0 : 10.0;
    UIView *weekView = [[UIView alloc]initWithFrame:CGRectMake(0, posY, MSS_SCREEN_WIDTH, MSS_WeekViewHeight +statusHeight)];
    ...
    CGFloat height = self.isPush ? MSS_WeekViewHeight : MSS_WeekViewHeight + 20.0;
    for(i = 0; i < 7;i++)
    {
...
- (void)back {
  if (self.showType == MSSCalendarShowTypeIsPresent) {
...
- (void)createUI
{
    ...
    CGFloat posY = self.isPush ? 64.0 : 0.0;
    _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, posY + MSS_WeekViewHeight, width * 7, MSS_SCREEN_HEIGHT - posY - MSS_WeekViewHeight) collectionViewLayout:flowLayout];
